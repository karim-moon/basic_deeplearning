...
class LinearRegression(object):
    ...
    def fit(self, X, y):
        ...
        if self.fit_intercept:
            intercepter_vector = np.ones([len(self._new_X), 1])
            self._new_X = np.concatenate((intercepter_vector, self._new_X),axis= 1)
    ...
    def predict(self, X):
        ...
        y : numpy array, 예측된 값을 1차원 vector 형태로 [n_predicted_targets]의
            구조를 가진다.
        """
        X_for_predict = np.array(X)
        if self.fit_intercept:
            intercept_vector = np.ones([len(X_for_predict),1])
            X_for_predict = np.concatenate((intercept_vector, X_for_predict), axis=1)

            weights = np.concatenate(([self._intercept], self._coef), axis=0)
        else:
            weights = self._coef
        return X_for_predict.dot(weights)
